...
- **v0.12.3（当前发布基准）**：2026-09-13 首发、**2026-09-14 同号重发**——版本号不变，只换构建，所以工坊与 GitHub 两侧各发布了两次。首发发布提交 `b0217b0`（`Release v0.12.3 for the Steam Workshop`，经 PR #108 合并，tag 当时指向它，先发工坊 22:31、后补 tag 与 Release）；重发把 tag 与 Release 重切到 `0f60ec4`（PR #113 的合并提交，内容 = #105 + #106 + #111），同日 00:09 重新上传工坊。GitHub Release 资产 `sts2-ai-agent-v0.12.3-windows.zip` 现为 552014 字节、SHA256 `B7684C9F2E8EAF7F43D9009EC5B6E0683033E9BC7088B52AEFDE5AE090DE5ADE`（首发那次 549933 字节 / `B9DC1A07…`）；工坊 `file_size` 现为 1229829（首发 1227269）。两份构建的**版本字符串相同**，只能靠大小或哈希区分——这是同号重发唯一的代价，CHANGELOG 的 v0.12.3 段顶部写明了这一点。见 [v0.12.3 发布记录](history/release-v0.12.3_2026-09-13.md)。**本版未单独做完整实机验收**：#106 的实机证据随身带来（未配置模型时真实点击拉起队友、`/health` 报 `companion.auto_play: false`），#111 的两个界面入口仍只有离线契约证据，待实机点击（见 `docs/live-validation-checklist.md` 的 `[coop]` 段）。
