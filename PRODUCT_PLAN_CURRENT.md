...
3. **`doc-links` 闸门**（第十一道）：402 个入库页面里的 445 条相对链接必须都解析得到。此前只有
   打进发布包的那三个文档被检。今天全绿，价值全在于它挡住的事——而本轮正好动了四个文件的位置。
4. **三处「在检查却没真检查」的东西被揪出来**，三处都早于本轮的拆分：
...
离线证据（本轮收口后全量重跑）：C# **415 PASS / 0 FAIL**；`mcp_server` **224 项 OK**；
`check_verification_gates.py` **十一道闸门全绿**；`test-verification-gates.ps1` **闸门自测 34 条全过**
...
给自测用例一个闸门永远不会说的期望、在入库页面里加一条断链——全部点名转红，随后逐字节还原。
