...
- **v0.12.3（当前发布基准）**：2026-09-13 发布，**先发工坊、随后补 tag 与 GitHub Release**。发布提交 `b0217b0`（`Release v0.12.3 for the Steam Workshop`）经 PR #108 合并，tag `v0.12.3` 指向该提交。工坊物品 3796486050 已更新：公开、`file_size` 1227269 与本地内容字节和相等、`time_updated` 2026-09-13 22:31:22、内容 id `5284257893537057643`；GitHub Release 资产 `sts2-ai-agent-v0.12.3-windows.zip`（549933 字节，SHA256 `B9DC1A070BE8A78ECFE1EC5ED37C6E606E4AA8CC2F8F383060EB3C953424B1B2`），GitHub 标记为 Latest。本版内容是 overlay 邀请路线修复（#106，作者 sachi4clover）与打包链接 gate（#105），见下方「v0.12.3 的内容」。见 [v0.12.3 发布记录](history/release-v0.12.3_2026-09-13.md)。**本版未单独做完整实机验收**：#106 的实机证据随身带来（未配置模型时真实点击拉起队友、`/health` 报 `companion.auto_play: false`），tag 的代码与那次验证逐字节相同。
