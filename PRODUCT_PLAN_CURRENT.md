...
> 本页是仓库唯一的当前状态入口。更新日期：2026-09-14（v0.12.3 **两次**同号重发；先是 #111 的两个联机界面入口完成实机点击验收，验收中发现并修掉一处 Continue 按钮刷新缺陷，随后把这处修复与当天的诊断 / 发布工具收口一起重切成第三次构建）。历史快照标记：本页为当前状态页，非历史快照。
> 发布代码基准：tag `v0.12.3` @ `c2630a8`（2026-09-14 第二次重切；此前依次指向 `0f60ec4`（第一次重发）与 `b0217b0`（2026-09-13 首发））；工坊与 GitHub 两侧同为 `0.12.3`。
