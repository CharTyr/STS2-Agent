...
> 本页是仓库唯一的当前状态入口。更新日期：2026-09-15（v0.12.4 **只更新工坊**，并在当日**两次同号重发**——第二次修掉战斗中一条一帧状态自相矛盾，第三次修掉第二次引入的主菜单 `/state` 失败；GitHub 侧仍停在 `v0.12.3`；此前 2026-09-14 是 v0.12.3 **两次**同号重发；先是 #111 的两个联机界面入口完成实机点击验收，验收中发现并修掉一处 Continue 按钮刷新缺陷，随后把这处修复与当天的诊断 / 发布工具收口一起重切成第三次构建）。历史快照标记：本页为当前状态页，非历史快照。
