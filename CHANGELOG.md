...
- **四项小优化。** 游玩页决策记录里，Jev 未能提交后由 LLM 接手的回合标注「Jev 未提交 → LLM 接手」（配英文）；新增 `GET /diagnostics`（仅 loopback，`text/plain`），不开覆盖层就能取到与「导出诊断」相同的脱敏文本；思考流式回调改为取累积器的有界预览（前 600 字符 + 省略号），长思考时不再每个增量全量重拼；Jev 面板新增本局「Jev 执行 X 次 · 回退 LLM Y 次」计数；规划器输入在极端大帧下不再退化为 `{}`，保底保留屏幕/回合/战斗骨架；截图的绘制帧等待改为显式 Connect/Disconnect，永不渲染的窗口不再每次失败截图泄漏一个信号连接；`[STS2AIAgent.Wide]` 宽度诊断改为 offenders 数量变化时上报（不再只报一次），后续动作引入的溢出不会再漏报。

- **双层模式的三个遗留缺陷补上，另有三项小优化。** LLM 回退路径现在能看到规划器的宏观目标：Jev 置信度不足交给慢模型时，提示词带上同一份 `goal` / `posture` / `instructions` / 按动作类别的提示（默认策略不耗 token，且挂在缓存前缀上）。规划器看到的状态不再是被字符截断的坏 JSON：`PlanningSummary` 按字段优先级裁剪（术语表/牌库/弃牌堆先让位，战斗事实最后），结果始终可解析——此前几乎每个战斗帧的规划器输入都无法解析，plan scope 恒为空。Jev 逐回合证据落地：决策日志的 `jev` 行现在带该帧枚举的全部选项 ID（封顶 48）、逐选项概率、危险度、所依据策略的 `updated_at`；Jev 未提交后由 LLM 接手的行标记 `jev_attempt=true`——2026-09-23 那串连续 `end_turn` 缺的就是这份证据，此后类似情况可以逐回合定性。
- **决策记录标记回退、诊断可走 HTTP、思考流式去 O(n²)。** 游玩页的决策记录里，Jev 未能提交后由 LLM 接手的行显示「Jev 未提交 → LLM 接手」，不再和普通 LLM 决策混在一行。新增 `GET /diagnostics`（仅 loopback，`text/plain`）：与覆盖层「导出诊断」按钮相同的脱敏文本，不开窗口也能排查问题。思考流式回调改为取有界预览（前 600 字符 + 省略号），不再每个 SSE delta 全量重拼累积文本——长思考时从 O(n²) 拷贝降为固定开销，最终完成体的完整思考不受影响（记录气泡与服务商回显照旧）。
...
- **The in-game prompt no longer re-sends the whole playbook on every step.** `PlaySystem` used to
  ...
  screens when one is relevant. The system prompt now carries the play contract, and the current
  screen's section arrives as its own message through the new `PlayPrompt.PlaybookGuidance(screen)`;
  a screen with no section gets a 622-character index of what exists rather than nothing, so the
  guidance is never silently empty. Measured on a COMBAT step: the static block drops from 24,105
  to 14,577 characters, about 2,722 tokens less per decision.

- **All static content now precedes all dynamic content in that message list.** The game state used
  to sit at message position 1, so every later message — guidance, team context, vision — was
  sent after a changing prefix and no provider could cache it. The state and the final "act once"
  instruction are now the last two messages, which makes the static prefix byte-identical across
  consecutive steps on one screen; a contract test pins that stability so a later edit cannot
  quietly move state back to the front.
...
- **The offline knowledge index gained the two files an agent asks for most.** There was no
  ...
  covers 289 relics (`Name | Rarity | Owner | Effect`) and `powers.md` covers 257 powers
  (`Name | Type | StackType | Hook | Effect`), both generated by the existing
  `scripts/generate-sts2-knowledge.ps1` rather than written by hand. Monsters stopped being the gap
  they were: MinHp/MaxHp went from 15 filled and 106 blank to all 121 filled, from single
  declarations only (the `AscensionHelper` fallback, an inherited property with the declaring class
...
- **`end_turn_will_kill_player` now counts damage that lands after you stop acting.** The flag was
  ...
  and constrict, which resolves when your own turn ends, were both invisible. A run could end with
  the flag still `false`, and `strategy.md` tells a model to trust it. Both now report a risk row
  (`poison_next_turn`, `constrict_turn_end`) with the damage, the damage after block and the reason;
...
> A visual and usability pass over the in-game window, plus the layout and theme work that came out
> of testing it in a running game. Six of the defects below were found by looking at the window and
> none of them was reachable from the offline suite, because each one is a question about what a
> running Godot scene does with the controls it is given.

- **The in-game window got a visual and usability pass, and seven colour themes instead of four.** The window now has depth — surfaces, cards and buttons carry shadows and a distinct border rather than reading as one flat slab — and section titles sit behind an accent bar, so a long page is scannable while a turn is running. The play tab is a dashboard rather than four label rows: the state is the largest thing on the page and turns green while the loop runs, and the screen, last action and token spend are three tiles beside it. The title bar carries the live state (`● Auto-playing` / `○ Idle`) instead of hiding it one tab away, the chat tab opens on a card that explains what to ask instead of an empty box, chat turns and decision-log entries colour the part that carries the meaning — the speaker, the action that was taken — and empty states say what will appear rather than showing a dash. The teammate line on the co-op tab is now a tone chip: green while the teammate is up, red the moment they are down, neutral while the first poll is still in flight. Three new themes join the four existing ones (`Crimson`, `Emerald`, `Amethyst`), and the theme picker is a grid of swatches rather than a drop-down — each tile draws the actual palette it selects, so choosing a colour scheme no longer means picking between seven words. Every preset is still checked offline against WCAG contrast floors — body text 4.5:1 and secondary text 3:1 on every surface it can be drawn on.

- **The long pages were rearranged around what a player reaches for first.** The settings page gained a jump bar (First run / Appearance / Endpoints / Models / Bindings) and its Save, Test and Add buttons now sit in a fixed footer instead of at the bottom of the scrolling form — changing the theme at the top of a form that grows with every configured endpoint no longer means scrolling past all of them to commit it. The co-op tab was inverted: the invite button, the continue entry and the teammate pause/resume row now come first and share one card, with the status card under them and team chat last, so pressing "pause teammate" mid-fight is no longer a scroll away; its chat log is sized from the panel height rather than pinned, so it gives up room before the action rows are pushed out of view. The chat footer's three switches share a single row and the message box is shorter, which returns most of that height to the message log. A jump button puts its section's heading at the top of the view, not against its bottom edge.

- **A light theme, and the theme picker now shows you the colours.** `Ivory` joins the presets: a warm off-white surface with near-black text and a deep bronze accent, for reading the panel in daylight or a bright room, which six dark presets cannot serve. Its accent is deliberately dark rather than bright — a bright accent cannot carry readable label text on a light surface (amber gives 3.5:1 against its own label; this one gives 7.3:1). The picker itself is a grid of swatches: each tile draws the palette it selects, with the accent as a full-height stripe and the content colours as bars taken from the theme's own text colours, so the bars stay legible on a dark theme instead of disappearing into it. Light themes draw no drop shadow at all and separate layers with borders instead; a shadow tuned for a near-black panel reads as a smudge on an off-white one.

- **The theme you click is the theme you keep.** Picking a swatch repaints the window immediately and writes the choice straight to the settings file, then leaves the rest of the page's edits to Save as before. Previously the swatch grid changed the stored theme only on Save, and because a theme change itself rebuilds the settings page, that rebuild could observe the pre-save value and write the default back over the choice that had just been made — pick a light theme, press Save, get the dark default.

- **Fixed six defects that only the live window could show.** None of these were reachable from the offline suite, because they are all questions about what a running Godot scene does with the controls it is given. The swatch preview never drew: it was a custom `_Draw` override that was never called, and tiles rendered as empty rectangles, so it is now built from plain child nodes. A live theme switch repainted labels and buttons but not `PanelContainer`s, so light cards sat on a still-dark chrome; every surface is now tagged with its style role where the style is applied, and the switch rebuilds each from the new palette. Long labels broke the layout: an unwrapped label reports its whole line as its minimum width, and one hint sentence measured 659 pixels against a 440-pixel panel, which pushed the content column to 691 and clipped every card against the panel edge. Altogether eleven static sentences and seven dynamic status lines now reflow, and the labels that can only get long at runtime — the ones that are `"-"` when built and a sentence once the game fills them in — are covered by a contract test so the class of bug cannot come back silently. The overlay's Save button and the six-item tab row were also left clipped or mis-coloured by the same root cause.

- **The overlay's own source files were split, and the ratchet came down with them.** `AgentOverlayHost.cs` was 1,415 lines against a 1,420-line budget when this work started, so the page bodies and the single 220-line refresh pass moved to `AgentOverlayHost.Pages.cs`, and the settings form moved on again to `AgentOverlayHost.Settings.cs`. The swatch renderer became `OverlaySwatch.cs` when the preview pushed `UiFactory.cs` past its own budget. Every one of those is a move, not a raise: the base file's budget came down from 1,420 to 1,150, and the architecture page's table and reasoning were re-measured to match. The layout contracts added by this work pin what the live pass established — that Save stays outside the scrolling form, that a jump lands at the top, that the co-op actions precede the co-op status, that long labels reflow, and that a theme switch repaints surfaces rather than only the text on them.
...
- **The in-game panel was redesigned, and it now has themes.** It was six pages of unstyled Godot defaults: one font size, grey buttons that looked the same whether they were the main action or a footnote, and no way to tell which tab you were on. There is now a small design system behind it — a spacing and type scale, section cards with headings, and buttons that distinguish a primary action, an ordinary one and a quiet one — and the header row draws the selected tab as a filled pill instead of leaving six identical labels to be read. Four colour themes ship with it (Dusk, Midnight, Parchment, High contrast), selectable in Settings under Appearance and applied immediately. Every theme is held to WCAG contrast floors by an offline test, against every surface the panel draws text on, so a pretty theme that is unreadable in a dark room fails the build rather than the player's eyes. The teammate page and the play page scroll now, which they needed: with cards they became taller than the window. Nothing about what the pages do changed.
