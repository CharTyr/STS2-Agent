...
internal static partial class Loc
{
    ...
    private static void AddSupportEntries(Dictionary<string, string> map)
    {
        ...
        map["连续等待游戏可操作超过 {0} 秒仍未恢复，已停止自动游玩。游戏可能停在需要手动处理的界面；处理后可手动继续。"] =
            "Waited for an actionable state for over {0} seconds without recovery, so auto-play stopped. The game may be sitting on a screen that needs a human; continue manually once it is handled.";
