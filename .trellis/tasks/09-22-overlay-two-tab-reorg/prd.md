...
- **单人模式内容**：
  ...
  - 对话输入框 + 发送 + 清空 + 「附带当前状态」「附带截图」两个勾选。2026-09-23 用户确认不恢复「允许代打」勾选：空闲聊天仅在明确提出代打意图时可代打一手，自动游玩中聊天只作为后续决策指令，不并发出牌。见 [已归档的审查修复需求](../archive/2026-09/09-23-audit-remediation/prd.md)。
...
### 设置页（settings）
- 保留现有：首次配置引导、外观（主题）、端点卡、模型卡、模型角色设置、高级选项（热键/预算/主动发言/窗口位置）。后续 `.trellis/tasks/09-22-pr196-live-fixes/plan.md:30-35` 明确将聊天与游玩合并为单一主模型；2026-09-23 用户确认维持该较新决定，不恢复独立 PlayModelId 选择器。
