...
A streamed body is read as it arrives rather than buffered whole. The SSE/JSON decision is made on the first non-blank line and then held (a provider that ignores `stream: true` is still parsed as JSON), complete lines go to [SseCompletionAccumulator](../../../STS2AIAgent/Llm/SseCompletionAccumulator.cs) -- which is also what `ParseSsePayload` feeds in one pass, so the two cannot drift -- and each reasoning delta is reported to `LlmRequest.OnReasoningDelta` before the reply finishes. Only reasoning is streamed: content, tool arguments, usage and the finish reason stay with the final completion, which is what the turn's legality checks and receipts read. The callback is display-only and a throw from it must never fail the request. [LiveThoughtBuffer](../../../STS2AIAgent/Agent/LiveThoughtBuffer.cs) is the only consumer: Godot-free, so the offline suite compiles the rules that matter (the player's switch gates storage, and a completed turn leaves nothing behind, which is what stops one turn from drawing two reasoning bubbles).
