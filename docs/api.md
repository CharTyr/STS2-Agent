...
每个客户端使用容量 256 的有界队列。队列满时服务端**不会静默丢弃旧事件，也不会阻塞 producer**，而是完成并关闭该慢客户端的 stream；客户端应重连，并以新的 `stream_ready`（或一次新 `/state`）重新对齐。`event_id` 全局递增；其他订阅的初始快照也会使用编号，因此单个订阅不保证编号连续。编号跳变不能单独证明丢失了广播事件，服务端也不提供历史补发；重连后应读取新快照。官方 Python `wait_for_event` 会在其原有总 deadline 内重连。
...
- `GET /strategy` 响应保留 `data.strategy`（当前策略：`posture` / `goal` / `instructions` / `option_hints` / `plan_screen` / `plan_round` / `updated_at` / `source`）、`data.dual_layer`（本实例的双层开关状态，host 读单人开关、companion 读多人开关）、`data.jev_configured`（Jev 的 base URL 与 key 是否已配齐）；新增只读 `data.run_summary`（与 `get_run_summary` 的 `run` 相同的原始状态摘要；无有效 `run_id` 时为 `null`）、`data.screen`（当前原始状态屏幕名，可空）、`data.recent_jev_decisions`（同一 `run_id` 最多 5 个 Jev 已接受决策，按时间从旧到新，仅 `id` / `timestamp` / `action` / `confidence`）、`data.confidence_trend`（有信心样本时含 `count`、`average`、`latest`、`direction`；否则 `null`）。`direction` 为 `rising` / `falling` / `steady`，单个样本为 `insufficient_data`；该趋势按最多 5 个近期 Jev 决策中具有 `confidence` 的样本计算。无局时决策列表为空，不借用上一局。此响应由与原生 MCP `get_planner_briefing` 相同的 C# 投影生成，Python MCP 直接转发 `/strategy` 的 `data`，不另行跨帧读取 `/state`/`/decisions`。不包含理由、提示词、Jev key 或请求体。
- `POST /strategy` 请求体 `{"strategy": {...}}`（或直接是策略对象）；`posture` / `goal` / `instructions` / `option_hints` 均可选，**省略的字段保留当前值**（与原生 MCP `update_play_strategy` 共用同一合并；此前本路由会把省略字段重置为默认值）。显式传入的 `option_hints` 整体替换旧表。`source` 记为 `mcp`、`updated_at` 记为当前时间。一个可识别且类型正确的字段都没有（含 `{}`）或不是对象时返回 400 `invalid_request`，不会把策略重置为默认
- 策略字段约定：`posture` 是整体倾向（如 `aggressive` / `defensive` / `balanced`）；`goal` 是一句话的宏观目标，放在每次 Jev 选择的最前面，超过 400 字符截断；`instructions` 是长期指导，**不要**点名具体卡牌下标或目标——它们每帧都变；`option_hints` 按**动作类别**（`play_card`、`end_turn`、`choose_map_node`…）写，执行端会映射到当前帧的具体选项 ID（如 `play_card:0->1`），写成下标的键会被丢弃
- `plan_screen` / `plan_round` 是只读来源信息：游戏内规划器记录该策略写于哪个屏幕与第几个战斗回合，Jev 请求据此标注跨战斗沿用的旧策略。外部写入不能设置它们；只改 `posture` 时保留，改写 `goal` / `instructions` / `option_hints` 时清空
...
| `source` | string | 写入者：`agent_loop`（游戏内自动游玩的 LLM 决策）、`jev`（双层模式下 Jev 执行模型的决策，带 `confidence`）、`http_api`（`POST /action`）、`native_mcp`（原生 MCP `act`） |
